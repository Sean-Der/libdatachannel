...
void DtlsTransport::SetTimerCallback(void *ctx, uint32_t int_ms, uint32_t fin_ms) {
	...
	dtlsTransport->int_ms = int_ms;
	dtlsTransport->fin_ms = fin_ms;

	if (fin_ms != 0) {
		dtlsTransport->timerSetAt = std::chrono::system_clock::now();
	}
}

int DtlsTransport::GetTimerCallback(void *ctx) {
	auto dtlsTransport = static_cast<DtlsTransport *>(ctx);
	auto elapsed = uint32_t(std::chrono::duration<double>(std::chrono::system_clock::now() - dtlsTransport->timerSetAt).count() * 1000);

	if (dtlsTransport->fin_ms == 0) {
		return -1;
	} else if (elapsed >= dtlsTransport->fin_ms) {
		return 2;
	} else if (elapsed >= dtlsTransport->int_ms) {
		return 1;
	} else {
		return 0;
	}
...
bool DtlsTransport::send(message_ptr message) {
	...
	do {
		...
		auto buff = &(message->data())[0];

		ret = mbedtls_ssl_write(&mSsl, (const unsigned char *) buff, message->size());
...
void DtlsTransport::doRecv() {
	...
	try {
		...
		// Handle handshake if connecting
		if (state() == State::Connecting) {
			auto handshakeStatus = mbedtls_ssl_handshake(&mSsl);
			switch (handshakeStatus) {
				...
				case MBEDTLS_ERR_SSL_CRYPTO_IN_PROGRESS: {
					auto timeout = milliseconds(this->fin_ms);
					ThreadPool::Instance().schedule(timeout, [weak_this = weak_from_this()]() {
						if (auto locked = weak_this.lock())
							locked->doRecv();
					});
		...
		if (state() == State::Connected) {
			while (true) {
				auto ret = mbedtls_ssl_read(&mSsl, (unsigned char *) buffer, bufferSize);
...
void DtlsTransport::ExportKeysCallback(void *p_expkey,
	...
	std::memcpy(dtlsTransport->masterSecret, secret, secret_len);
	std::memcpy(dtlsTransport->randBytes, client_random, 32);
	std::memcpy(dtlsTransport->randBytes + 32, server_random, 32);
	dtlsTransport->tlsProfile = tls_prf_type;
}
...
int DtlsTransport::ReadCallback(void *ctx, unsigned char *buf, size_t len) {
	...
	try {
		while (t->mIncomingQueue.running()) {
			...
			len = std::min(len, size_t(message->size()));
