...
DtlsTransport::DtlsTransport(shared_ptr<IceTransport> lower, certificate_ptr certificate,
	...
	try {
		mbedtls::check(mbedtls_ctr_drbg_seed(&mDrbg, mbedtls_entropy_func, &mEntropy, NULL, 0), "Failed creating Mbed TLS Context");

		mbedtls::check(mbedtls_ssl_config_defaults(
					&mConf, mIsClient ? MBEDTLS_SSL_IS_CLIENT : MBEDTLS_SSL_IS_SERVER,
					MBEDTLS_SSL_TRANSPORT_DATAGRAM, MBEDTLS_SSL_PRESET_DEFAULT), "Failed creating Mbed TLS Context");
		...
		mbedtls::check(mbedtls_ssl_conf_own_cert(&mConf, crt.get(), pk.get()), "Failed creating Mbed TLS Context");
...
bool DtlsTransport::send(message_ptr message) {
	...
	do {
		...
		ret = mbedtls_ssl_write(&mSsl, reinterpret_cast<const unsigned char *>(message->data()), message->size());
	} while (ret == MBEDTLS_ERR_SSL_WANT_WRITE);
	mbedtls::check(ret, "Failed writing to Mbed TLS Context");
...
void DtlsTransport::doRecv() {
	...
	try {
		...
		if (state() == State::Connecting) {
			...
			{
				auto timeout = milliseconds(this->mFin_ms);
				ThreadPool::Instance().schedule(timeout, [weak_this = weak_from_this()]() {
					if (auto locked = weak_this.lock())
						locked->doRecv();
				});
			...
			mbedtls::check(ret, "Failed Mbed DTLS Handshake");
			PLOG_INFO << "DTLS handshake finished";
			changeState(State::Connected);
			postHandshake();
		}
		...
		if (state() == State::Connected) {
			while (true) {
				mMutex.lock();
				auto ret = mbedtls_ssl_read(&mSsl, reinterpret_cast<unsigned char *>(buffer), bufferSize);
				mMutex.unlock();
				...
				mbedtls::check(ret, "Failed Mbed DTLS Read");
...
void DtlsTransport::ExportKeysCallback(void *p_expkey,
	...
	auto dtlsTransport = static_cast<DtlsTransport *>(p_expkey);
...
void DtlsTransport::SetTimerCallback(void *ctx, uint32_t int_ms, uint32_t fin_ms) {
	...
	dtlsTransport->mInt_ms = int_ms;
	dtlsTransport->mFin_ms = fin_ms;
...
int DtlsTransport::GetTimerCallback(void *ctx) {
	auto dtlsTransport = static_cast<DtlsTransport *>(ctx);
	auto elapsed = std::chrono::steady_clock::now() - dtlsTransport->mTimerSetAt;
	auto ms = uint32_t(std::chrono::duration_cast<std::chrono::milliseconds>(elapsed).count());

	if (dtlsTransport->mFin_ms == 0) {
		return -1;
	} else if (ms >= dtlsTransport->mFin_ms) {
		return 2;
	} else if (ms >= dtlsTransport->mInt_ms) {
		return 1;
	} else {
		return 0;
	}
