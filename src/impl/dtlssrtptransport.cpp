...
void DtlsSrtpTransport::postHandshake() {
	...
	if (mbedtls_ssl_tls_prf(mTlsProfile, reinterpret_cast<const unsigned char *>(mMasterSecret), sizeof(mMasterSecret), label.c_str(),
				reinterpret_cast<const unsigned char *>(mRandBytes), sizeof(mRandBytes), material.data(), materialLen) != 0) {
